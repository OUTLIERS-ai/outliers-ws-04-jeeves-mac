...
False today (every member is sent to the repos that exist), True from wave 6 (a Mac member is sent
to the Mac copies). These checks run on every system.
...
def test_the_setting_is_1_line_and_off_until_the_mac_repos_exist():
    lines = setting_lines((ROOT / "jeeves" / "config.py").read_text(encoding="utf-8"))
    assert lines == ["False"], "jeeves/config.py must set MAC_REPOS_PUBLISHED exactly once, to False: %r" % lines
...
def test_today_every_member_is_sent_to_a_repo_that_exists():
    from jeeves import config as C
    for mac in (True, False):
        for name in APPS.values():
            assert C.app_repo(name, mac=mac) == BASE + name
    assert {k: v["repo"] for k, v in C.DEFAULTS["apps"].items()} == {k: BASE + n for k, n in APPS.items()}
